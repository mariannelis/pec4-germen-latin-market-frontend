...
const FONSO_AVATAR = "/images/fonso-avatar.png";
...
function Fonso({ open = false, onOpenChange }) {
  ...
  const topics = {
    pedido: {
      label: "Pedido",
      answer:
        "Agrega productos al carrito, revisa cantidades y tramita tu pedido por WhatsApp. Yo te acompaño para confirmar disponibilidad, entrega y forma de pago.",
    },
    envio: {
      label: "Envíos",
      answer:
  ...
  const whatsappUrl = useMemo(() => {
    const message = "Hola Fonso, vengo desde Germen Latin Market y necesito ayuda con mi compra.";
  ...
  return (
    <div id="asistente-fonso" className="fixed bottom-5 right-5 z-50">
      {isOpen && (
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="mt-1 font-serif text-2xl font-bold">Fonso</h3>
                  <p className="text-xs text-white/80">Tu asistente de compra</p>
                </div>
              </div>
              <button
                type="button"
                ...
                aria-label="Cerrar asistente Fonso"
              >
                ×
              </button>
            </div>
          ...
          <div className="space-y-4 bg-[#fff8f7] p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="rounded-3xl rounded-tl-md bg-white p-4 text-sm leading-6 text-[#5f454b] shadow-sm">
                Hola, soy Fonso. ¿Te ayudo a encontrar productos, revisar tu carrito o tramitar tu pedido?
              </div>
            </div>
      ...
      <button
        ...
        aria-label="Abrir asistente Fonso"
      >
        <img src={FONSO_AVATAR} alt="Fonso" className="h-full w-full object-cover" />
        ...
        <span className="absolute bottom-full right-0 mb-3 hidden whitespace-nowrap rounded-2xl bg-white px-4 py-2 text-xs font-black text-[#8a0f2a] shadow-lg group-hover:block">
          ¿Necesitas ayuda? Habla con Fonso
        </span>
      </button>
    </div>
  );
